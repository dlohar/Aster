...
# This script runs Aster's setting A to quantize all networks and subsequently
# runs Xilinx to evaluate the generated code.

# generate output directory
if [ ! -d Aster-SettingA ]; then
 mkdir Aster-SettingA
fi

output_folder="Aster-SettingA"
...
echo "... running Aster with setting A"
# Running benchmarks with their corresponding layers
for file in "${benchmarks[@]}"
do
 echo "****************************** Benchmark:" ${file} "******************************"
 timeout 300m ./daisy --qnn --qnn-quant --precision=Fixed32 --layer=${layers[$i]} --minLen=5 --maxLen=32 --initLen=20 --lang=C --apfixed ${input_file_path}/${file}.scala
