...
# This script runs Aster's setting C to quantize all networks and subsequently
# runs Xilinx to evaluate the generated code.

# generate output directory
if [ ! -d Aster-SettingC ]; then
 mkdir Aster-SettingC
fi

output_folder="Aster-SettingC"
...
echo "... running Aster with setting C"
# Running benchmarks with their corresponding layers
for file in "${benchmarks[@]}"
do
 echo "****************************** Benchmark:" ${file} "******************************"
 timeout 300m ./daisy --qnn --qnn-quant --precision=Fixed32 --layer=${layers[$i]} --minLen=5 --maxLen=32 --initLen=10 --noInitError --lang=C --apfixed ${input_file_path}/${file}.scala
