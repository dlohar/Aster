...
# This script runs Aster's setting B to quantize all networks and subsequently
# runs Xilinx to evaluate the generated code.

# generate output directory
if [ ! -d Aster-SettingB ]; then
 mkdir Aster-SettingB
fi

output_folder="Aster-SettingB"
...
echo "... running Aster with setting B"
# Running benchmarks with their corresponding layers
for file in "${benchmarks[@]}"
do
 echo "****************************** Benchmark:" ${file} "******************************"
 timeout 300m ./daisy --qnn --qnn-quant --precision=Fixed32 --layer=${layers[$i]} --minLen=10 --maxLen=32 --initLen=32 --lang=C --apfixed ${input_file_path}/${file}.scala
