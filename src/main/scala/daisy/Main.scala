...
// "Quad" -> DoubleDouble, "QuadDouble" -> QuadDouble,
  ...
  //      "Fixed32" -> FixedPrecision(32), "Fixed64" -> FixedPrecision(64)),
    ...
    FlagOption("qnn", "input is a neural network"),
    FlagOption("qnn-quant", "quantize a neural network"),
    FlagOption(
      "nncodegen",
      "Generate NN code (as opposed to just doing analysis)")
  ...
  private def computePipeline(ctx: Context): Pipeline[Program, Program] = {
    ...
    if (ctx.hasFlag("qnn")) {
      pipeline >>= transform.NNUnrollPhase
    }
    ...
    if (ctx.hasFlag("dynamic")) {
      ...
      if (ctx.hasFlag("nncodegen")) {
        pipeline >>= backend.NNCodeGenerationPhase
      }
    }
