...
trait CodeExtraction extends ASTExtractors {
  ...
  class Extraction(units: List[CompilationUnit]) {
    ...
    private def isLibrary(u: CompilationUnit) = {
      // TODO: big hack
      u.source.file.absolute.path.endsWith(new File(self.ctx.libFiles(0)).getAbsolutePath()) ||
    ...
    private def extractTree(tr: Tree)(implicit dctx: DefContext): DaisyExpr = {
      ...
      val res = current match {
        ...
        case ExTanh(e)          => Tanh(extractTree(e))
        case ExRelu(e)        => Relu(extractTree(e))
        case ExLinear(e)        => Linear(extractTree(e))
        ...
        case ExMatrixLiteral(args) =>
          val data = args.map(row => row.map(elem => {
            extractTree(elem) match {
        ...
        case ExVectorLiteral(args) =>
          val data = args.map(elem => {
            extractTree(elem) match {
              case RealLiteral(r) => r
              case _ => null
            }
        ...
        case ExCall(tr, sym, args) if (args.length == 1) =>
          ...
          (lhs, sym.name.decoded, rhs) match {
            ...
            case (IsTyped(a1, MatrixType), "*", IsTyped(a2, VectorType)) =>
              LinearMap(a1, a2)
            case (IsTyped(a1, VectorType), "+", IsTyped(a2, VectorType)) =>
              VectorPlus(a1, a2)              
        ...
        case ExLowerBounds(t, args) =>
          ...
          LowerBounds(extractTree(t), data)

        case ExUpperBounds(t, args) =>
          val data = args.map(elem => {
            extractTree(elem) match {
              case RealLiteral(r) => r
              case _ => null
          ...
          UpperBounds(extractTree(t), data)
