...
trait ASTExtractors {
  ...
  object StructuralExtractors {
    ...
    object ExImplicitDouble2Real {
      def unapply(tree: Apply): Option[Double] = tree match {
        case Apply(select, List(Literal(c @ Constant(i)))) if (select.toString == "daisy.lang.Real.double2real") =>
          Some(c.doubleValue)
        // for some reason for Vector's the path is different
        case Apply(select, List(Literal(c @ Constant(i)))) if (select.toString == "lang.this.Real.double2real") =>
          Some(c.doubleValue)
        case _ => None
      }
    }
    ...
    object ExTanh {
      def unapply(tree: Apply): Option[Tree] = tree match {
        case Apply(select, List(arg)) if (select.toString == "daisy.lang.Vector.tanh") => Some(arg)
        case _ => None
      }
    }
    ...
    object ExLowerBounds {
      def unapply(tree: Apply): Option[(Tree, List[Tree])] = tree match {
        case Apply(select, List(t, bounds)) if (select.toString == "daisy.lang.Vector.lowerBounds") =>
          bounds match {
            case Apply(select, args) if (select.toString == "immutable.this.List.apply[Double]") =>
              Some((t, args))
            case _ => None
          }
        case _ => None
    ...
    object ExUpperBounds {
      def unapply(tree: Apply): Option[(Tree, List[Tree])] = tree match {
        case Apply(select, List(t, bounds)) if (select.toString == "daisy.lang.Vector.upperBounds") =>
          bounds match {
            case Apply(select, args) if (select.toString == "immutable.this.List.apply[Double]") =>
              Some((t, args))
            case _ => None
    ...
    object ExMatrixLiteral {
      def unapply(tree: Apply): Option[List[List[Tree]]] = tree match {
        case Apply(s @ Select(t, _), args) if (t.toString == "new daisy.lang.Matrix") =>
          
          args.head match {
            case Apply(select, args) if (select.toString == "immutable.this.List.apply[List[Double]]") =>
              
              val data = args.map(t => t match {
                case Apply(select, iargs) if (select.toString == "immutable.this.List.apply[Double]") =>
                  iargs
              })
    ...
    object ExVectorLiteral {
      def unapply(tree: Apply): Option[List[Tree]] = tree match {
        case Apply(s @ Select(t, _), args) if (t.toString == "new daisy.lang.Vector") =>
    ...
    object ExRelu {
      def unapply(tree: Apply): Option[Tree] = tree match {
        case Apply(select, List(arg)) if (select.toString == "daisy.lang.Vector.relu") => Some(arg)
        case _ => None
      }
    }
    ...
    object ExLinear {
      def unapply(tree: Apply): Option[Tree] = tree match {
        case Apply(select, List(arg)) if (select.toString == "daisy.lang.Vector.linear") => Some(arg)
        case _ => None
      }
    }
