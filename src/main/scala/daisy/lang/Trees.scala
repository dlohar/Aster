...
//    def Pow(that: Expr): Expr = Trees.Pow(this, that)
  ...
  case class MatrixLiteral(value: List[List[Rational]]) extends Literal[List[List[Rational]]] {
    val getType = MatrixType
  }

  case class VectorLiteral(value: List[Rational]) extends Literal[List[Rational]] {
    val getType = VectorType
  }

  // used at the end of NN programs to select the largest error from a vector
  case class Max(values: Seq[Expr]) extends Expr {
    val getType = RealType
  }
...
//  }
  ...
  case class Tanh(t: Expr) extends Expr {
    val getType = t.getType
  }
  ...
  case class LinearMap(lhs: Expr, rhs: Expr) extends Expr {
    //assert(lhs.getType == MatrixType && rhs.getType == VectorType)
    val getType = VectorType
  }

  case class VectorPlus(lhs: Expr, rhs: Expr) extends Expr {
    //assert(lhs.getType == VectorType && rhs.getType == VectorType)
    val getType = VectorType
  }

  case class Relu(t: Expr) extends Expr {
    val getType = t.getType
  }

  case class Linear(t: Expr) extends Expr {
    val getType = t.getType
  }
  ...
  /* Neural networks */
  case class LowerBounds(t: Expr, bounds: List[Rational]) extends Expr {
    val getType = BooleanType
  }

  case class UpperBounds(t: Expr, bounds: List[Rational]) extends Expr {
    val getType = BooleanType
  }
