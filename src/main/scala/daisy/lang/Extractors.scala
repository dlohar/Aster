...
//        Some(Seq(t1, t2), (es: Seq[Expr]) => Pow(es(0), es(1)))
      ...
      case Tanh(t) =>
        Some((Seq(t), (es: Seq[Expr]) => Tanh(es.head)))
      ...
      case Relu(t) =>
        Some((Seq(t), (es: Seq[Expr]) => Relu(es.head)))
      case Linear(t) =>
        Some((Seq(t), (es: Seq[Expr]) => Linear(es.head)))
      case VectorPlus(t1, t2) =>
        Some(Seq(t1, t2), (es: Seq[Expr]) => VectorPlus(es(0), es(1)))
      case LinearMap(t1, t2) =>
        Some(Seq(t1, t2), (es: Seq[Expr]) => LinearMap(es(0), es(1)))
