...
//  def ^(n: SMTRange): SMTRange = this ^ (n, lowPrecision, lowLoopThreshold)
  ...
  def tanh: SMTRange = this.tanh(lowPrecision, lowLoopThreshold)
...
//  }
  ...
  def tanh(precision: Rational = lowPrecision, maxLoops: Int = lowLoopThreshold): SMTRange = {
    val newTree = Log(this.tree)
    val newInterval = tightenBounds(this.interval.tanh, newTree, this.precond, this.constraints,
      precision, maxLoops)
    SMTRange(newInterval, newTree, this.precond, this.constraints)
  }
