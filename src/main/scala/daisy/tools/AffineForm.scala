...
case class AffineForm(x0: Rational, noise: Seq[Deviation]) extends RangeArithmetic[AffineForm] {
  ...
  def tanh: AffineForm = {
    ...
    val alpha = if (toInterval.includes(Rational.zero)){
      // largest slope (1.0) at 0.0
      one
    } else {
      // otherwise, the largest slope is at one of the boundaries
      val slopeLo = 1 - tanhDown(a)*tanhDown(a)
      val slopeHi = 1 - tanhUp(b)*tanhUp(b)
      assert(slopeLo >= Rational.zero && slopeHi >= Rational.zero)
      max(slopeLo, slopeHi)
    }

    val dmin = tanhDown(a) - (alpha * a)
    val dmax = tanhUp(b) - (alpha * b)
