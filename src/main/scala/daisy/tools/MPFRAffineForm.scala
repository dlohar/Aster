...
case class MPFRAffineForm(x0: MPFRInterval, noise: Seq[MPFRDeviation]) extends RangeArithmetic[MPFRAffineForm]{
  ...
  def tanh: MPFRAffineForm = {
    val (a, b) = (toMPFRInterval.xlo, toMPFRInterval.xhi)

    val alpha = if (toMPFRInterval.includes(MPFRFloat.zero)){
      // largest slope (1.0) at 0.0
      MPFRInterval(one)
    } else {
      // otherwise, the largest slope is at one of the boundaries
      val slopeLo = one - tanhDown(a)*tanhDown(a)
      val slopeHi = one - tanhUp(b)*tanhUp(b)
      assert(slopeLo >= MPFRFloat.zero && slopeHi >= MPFRFloat.zero)
      MPFRInterval(max(slopeLo, slopeHi))
    }

    val dmin = MPFRInterval(a).tanh - (alpha * a)
    val dmax = MPFRInterval(b).tanh - (alpha * b)
