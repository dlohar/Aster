      ...
      if (choosePrecision != "no") {
        ...
        val availablePrecisions = choosePrecision match {
          case "fixed" =>
            ...
            (start to maxBits).map(x => FixedPrecision(x))
    ...
    (rangeMethod: @unchecked) match {
      ...
      case "intervalMPFR" =>
        val (rng, intrmdRange) = evalRange[MPFRInterval](expr,
          inputValMap.mapValues(MPFRInterval(_)), MPFRInterval.apply)
        (rng.toInterval, intrmdRange.mapValues(_.toInterval))

      case "affineMPFR" =>
        val (rng, intrmdRange) = evalRange[MPFRAffineForm](expr,
          inputValMap.mapValues(MPFRAffineForm(_)), MPFRAffineForm.apply)
        (rng.toInterval, intrmdRange.mapValues(_.toInterval))
    }
  }
    ...
    (errorMethod: @unchecked) match {
      ...
      case "intervalMPFR" =>
        ...
        val (resRoundoff, allErrors) = evalRoundoff[MPFRInterval](expr, intermediateRanges,
          precisionMap,
          inputErrorMap.mapValues(MPFRInterval.+/-),
      ...
      case "affineMPFR" =>
        ...
        val (resRoundoff, allErrors) = evalRoundoff[MPFRAffineForm](expr, intermediateRanges,
          precisionMap,
          inputErrorMap.mapValues(MPFRAffineForm.+/-),
