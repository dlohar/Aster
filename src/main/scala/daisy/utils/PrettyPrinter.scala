...
object PrettyPrinter {
  ...
  def withTypes(t: Tree): String = {
    val printer = new PrettyPrinter(printTypes = true)
    printer.pp(t, None)(0)
    printer.toString
  }
}
...
class PrettyPrinter(val sb: Appendable = new StringBuffer, printUniqueIds: Boolean = false,
  ...
  def pp(tree: Tree, parent: Option[Tree])(implicit lvl: Int): Unit = {
    ...
    tree match {
      ...
      case LowerBounds(t, bounds) => sb.append(bounds.mkString("[", ",", "]") + " <= " + t)
      case UpperBounds(t, bounds) => sb.append(t.toString + " <= " + bounds.mkString("[", ",", "]"))
      ...
      case Tanh(expr) => ppMathFun(Seq(expr), "tanh")
      case Approx(_, expr, _, _, fName, _) => ppMathFun(Seq(expr), fName)
      case Relu(expr) => ppMathFun(Seq(expr), "relu")
      case Linear(expr) => ppMathFun(Seq(expr), "linear")
      ...
      case VectorPlus(l, r) => ppBinary(l, r, " + ")
      case LinearMap(l, r) => ppBinary(l, r, " * ")
      ...
      case VectorType => sb.append("Vector")
      case MatrixType => sb.append("Matrix")
