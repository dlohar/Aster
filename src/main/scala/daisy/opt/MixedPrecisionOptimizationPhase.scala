  ...
  override def runPhase(ctx: Context, prg: Program): (Context, Program) = {
    ...
    val availablePrecisions = defaultPrecision match {
      case FixedPrecision(b) => // given precision specifies the upper bound
        (start to b).map(FixedPrecision(_))
      case _ =>
    ...
    val newDefs: Seq[FunDef] = transformConsideredFunctions(ctx, prg){ fnc => {
      ...
      val newReturnType = fnc.returnType match {
        ...
        case _ => FinitePrecisionType(resPrecision)
      }
  ...
  */
  def computeAbsError(expr: Expr, precMap: Map[Identifier, Precision],
    ...
    def eval(e: Expr, path: PathCond): (AffineForm, Precision) = e match {
      ...
      case Let(id, x @ Tanh(y @ Variable(t)), body) =>
        val rangeT = rangeMap(y, path)
        ...
        val errorMultiplier = if (a >= Rational.zero) {
          // The interval is fully above zero, so the maximum derivative is at a 
          1 - Rational.tanhDown(a)*tanhDown(a)
        } else if (b <= Rational.zero) {
          // The interval is fully below zero, so the maximum derivative is at b
          1 - Rational.tanhUp(b)*tanhUp(b)
        } else {
          // The interval contains zero, so the maximum value is 1
          Rational.one
        }
        val propError = errorT * errorMultiplier
      ...
      case Let(id, x @ Relu(y @ Variable(t)), body) =>
        // relu does not affect the error, but may not preserve correlations,
        // so we remove them:
        var newError = AffineForm((eval(y, path)._1).toInterval)
      ...
      case Let(id, x @ Linear(y @ Variable(t)), body) =>
        // propagated error
        var newError = eval(y, path)._1
      ...
      // expanded tuple, return highest error
      case Let(id, y @ Variable(t), body) if (id.toString.startsWith("#res")) =>
        val errorT = eval(y, path)
        val errorBody = eval(body, path)

        if (Interval.maxAbs(errorT._1.toInterval) > Interval.maxAbs(errorBody._1.toInterval)) {
          errorT
        } else {
          errorBody
        }
